...
function PartMesh({ part, jointPreview, jointType }: { part: Part; jointPreview?: { position: [number, number, number]; rotation: [number, number, number] }, jointType?: string }) {
    ...
    return (
        <>
            ...
            {isSelected && !jointCreationMode && (!hasPreview || (jointType === 'slide' || jointType === 'cylindrical')) && meshRef.current && (
                <TransformControls
...
function Scene() {
    ...
    const jointPreviews = useMemo(() => {
        const previews = new Map<string, { transform: { position: [number, number, number]; rotation: [number, number, number] }, type: string }>();
        ...
        for (const joint of joints) {
            ...
            const transform = getJointPreviewTransform(joint, parentPart, childPart);
            previews.set(joint.childId, { transform, type: joint.type });
        }
        ...
        <>
            ...
            {parts.map((part) => (
                <PartMesh
                    key={part.id}
                    part={part}
                    jointPreview={jointPreviews.get(part.id)?.transform}
                    jointType={jointPreviews.get(part.id)?.type}
                />
            ))}
