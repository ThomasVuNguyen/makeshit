...
function jointTypeToMJCF(type: JointType): string {
    switch (type) {
        ...
        case 'cylindrical': return 'slide'; // Fallback, handled explicitly in generation
    }
}
...
function generateBodyXML(node: BodyNode, indent: string = '    '): string {
    ...
    if (jointToParent && jointToParent.type !== 'fixed') {
        ...
        if (joint.type === 'cylindrical') {
            // Composite joint: Slide + Hinge
            // Slide
            xml += `${indent}  <joint name="${jointName}_slide" type="slide" axis="${axis}" range="${joint.limitLower.toFixed(4)} ${joint.limitUpper.toFixed(4)}"/>\n`;
            // Hinge (Free rotation or limited? Usually cylindrical allows free rotation unless constrained)
            // For now, let's assume free rotation or full range -PI to PI
            xml += `${indent}  <joint name="${jointName}_hinge" type="hinge" axis="${axis}"/>\n`;
        } else {
