    ...
    temporal(
        (set, get) => ({
            ...
            updatePartPosition: (id, position) => {
                ...
                // CHECK FOR JOINT CONSTRAINT (Cylindrical/Slide)
                // If this part is a child of a joint that has a pivot set, we constrain the movement
                const controllingJoint = joints.find(j => j.childId === id && j.pivotSet && (j.type === 'slide' || j.type === 'cylindrical'));

                if (controllingJoint) {
                    const parentPart = parts.find(p => p.id === controllingJoint.parentId);
                    if (!parentPart) return;

                    // Calculate axis in world space
                    const axisVec = new THREE.Vector3();
                    if (controllingJoint.axis === 'x') axisVec.set(1, 0, 0);
                    if (controllingJoint.axis === 'y') axisVec.set(0, 1, 0);
                    if (controllingJoint.axis === 'z') axisVec.set(0, 0, 1);
                    // Axis is local to PARENT? Usually joints are defined in parent frame or child frame.
                    // In our getJointPreviewTransform, we simply add to the PIVOT which is computed from parent position.
                    // The axis logic in getJointPreviewTransform:
                    // position[axisIndex] += currentValue; 
                    // This implies the axis is World Aligned ??
                    // Looking at getJointPreviewTransform:
                    // const pivotWorld = parentPart.position + joint.anchorPosition
                    // position[axisIndex] += currentValue
                    // Yes, currently the axis is simply X/Y/Z global (or parent-aligned if we assume parent has 0 rotation? Wait.)

                    // Actually, looking at getJointPreviewTransform lines 48-56:
                    // It modifies `position[axisIndex]`. `position` starts as `pivotWorld`.
                    // So it is moving along the WORLD axes X/Y/Z.
                    // This implies our joints are currently World-Axis aligned, or we are assuming they are.
                    // If the parent rotates, `pivotWorld` changes location, but the slide direction `axisIndex` is still just indexing the vector.
                    // If the parent is rotated, the visual slide will be wrong if we don't rotate the axis.
                    // BUT, `getJointPreviewTransform` returns `{ position, rotation: childPart.rotation }`.
                    // It does NOT apply parent rotation to the axis.
                    // So for now, we assume World Axis for the slide.

                    // Logic:
                    // 1. Calculate projected movement along the axis.
                    // 2. Convert to previewValue delta.
                    // 3. Update Joint.

                    // We need the DELTA of the movement.
                    // The gizmo gives us the NEW target position.
                    // But wait, the standard updatePartPosition calculates delta:
                    const oldPosition = new THREE.Vector3(...parts[partIndex].position);
                    // Wait, `part.position` is the REST position (before preview transform).
                    // The Gizmo is actively moving the VISUAL mesh, which is at the PREVIEW position.
                    // So the `position` argument passed here is the NEW PREVIEW position.

                    // We need the CURRENT visual position to calculate delta.
                    // Or we can just project the NEW position onto the axis relative to the pivot.

                    const pivotWorld = new THREE.Vector3(
                        parentPart.position[0] + controllingJoint.anchorPosition[0],
                        parentPart.position[1] + controllingJoint.anchorPosition[1],
                        parentPart.position[2] + controllingJoint.anchorPosition[2]
                    );

                    const newPosVec = new THREE.Vector3(...position);

                    // Vector from Pivot to New Position
                    const vec = newPosVec.clone().sub(pivotWorld);

                    // Project onto Axis
                    // Since axis is world-aligned X/Y/Z:
                    let projectedValue = 0;
                    if (controllingJoint.axis === 'x') projectedValue = vec.x;
                    if (controllingJoint.axis === 'y') projectedValue = vec.y;
                    if (controllingJoint.axis === 'z') projectedValue = vec.z;

                    // Update Preview Value
                    // currentValue = limitLower + range * previewValue
                    // projectedValue is the new relative position (offset from pivot).
                    // So projectedValue IS the new 'currentValue' (distance from pivot).

                    const range = controllingJoint.limitUpper - controllingJoint.limitLower;
                    if (range === 0) return; // Divide by zero protection

                    // projectedValue = limitLower + range * newPreviewValue
                    // newPreviewValue = (projectedValue - limitLower) / range

                    let newPreviewValue = (projectedValue - controllingJoint.limitLower) / range;

                    // Clamp
                    newPreviewValue = Math.max(0, Math.min(1, newPreviewValue));

                    // Update Joint
                    get().updateJoint(controllingJoint.id, { previewValue: newPreviewValue });
                    return; // SKIP STANDARD PART UPDATE
                }
